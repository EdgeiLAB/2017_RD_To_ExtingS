...
void callbackArrived(char * topicName, uint8_t * payload, unsigned int len)
{
  ...
  if(rc==MQTTCLIENT_SUCCESS )
  {
    ...
    case GET_LATEST_DATA_REQUESTED:
    {
      ...
      printf("latestData callback : %s \t %s \t %s \n", strRi, notificationName.c_str(), strCon);
  ...
  latestDataVector.erase(latestDataVector.begin(), latestDataVector.end());
  
  callback.clear(); 
  memset(strRsc, 0, 128);
  memset(strRi, 0, 128);
  memset(strCon, 0, 128);
  memset(strRoute, 0, 128);
  memset(notifySubName, 0, 128);    
  callbackFunction = NULL;
...
int mqttCreateContentInstance(PubSubClient* client, const char* con, char * dataValue)
{
  ...
  mqttRemoteCSE = new char[BUF_SIZE_LARGE];
  mqttContainer = new char[BUF_SIZE_LARGE];
  int rc = 0;
  char container[BUF_SIZE_SMALL];
  char strRi[128];
  generateRi(strRi);
